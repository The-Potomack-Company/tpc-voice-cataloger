...
- **[D-043](../../_workspace/Decisions/D-043-gcp-consolidation-retarget.md)** — Retarget the long-term hosting consolidation from AWS (D-039) to the existing TPC-owned Google Cloud project. Migration stays deferred until after v3.0 hub cutover + 30-day legacy archive — same sequencing constraint D-039 set. New drivers vs D-039 AWS pick — a TPC-owned GCP project already exists, and Gemini (cataloger + extension AI) already bills to Google — so GCP unifies AI + hosting + DB under one account. v3.0 ships unchanged on Vercel + Supabase + Cloudflare; GCP migration becomes a dedicated post-v3.0 consolidation milestone.
