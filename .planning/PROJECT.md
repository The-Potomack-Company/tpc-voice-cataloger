...
- **[D-030](../../_workspace/Decisions/D-030-staging-supabase-project.md)** — Staging gets a separate Supabase project (tpc-staging). All staging deployments route there. Prod migrations run post-staging-verification with approval gate. Complete data isolation between dev/test and production.
- **[D-031](../../_workspace/Decisions/D-031-dev-email-staging-redirect.md)** — josh@potomackco.com is hardcoded to redirect to tpc-staging Supabase on login. Hard email-based redirect, not row-level filtering. Simple and durable; avoids RLS gymnastics for one dev account.
- **[D-032](../../_workspace/Decisions/D-032-staging-cicd-prod-approval-gate.md)** — DB migrations auto-run on staging on every merge. Prod migrations require a manual approval click in GitHub Actions before applying. Staging verification serves as the human review; prod gate adds a 5sec safety net for risky SQL.
